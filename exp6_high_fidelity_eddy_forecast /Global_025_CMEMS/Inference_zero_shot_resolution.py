...
# Dimensions:    (time: 365, latitude: 1440, longitude: 2880)
...
try:
    ...
    # It is recommended to add weights_only=True when using torch.load for better security, unless you are sure the file source is reliable
...
inference_data_path = os.path.join(config['data_root'], f"SSH_low_{config['inference_year']}.nc")
print(f"Loading data for the entire year {config['inference_year']} from: {inference_data_path}")

# --- Constructing real coordinates ---
inference_year = config['inference_year']
# A more robust way to check for a leap year
is_leap = (inference_year % 4 == 0 and inference_year % 100 != 0) or (inference_year % 400 == 0)
...
lat_coords_real = np.linspace(89.875, -89.875, 1440)
lon_coords_real = np.linspace(0.125, 359.875, 2880)
...
try:
    with xr.open_dataset(inference_data_path) as ds:
        ...
        for start_day in tqdm(start_days, desc="Processing Initial Conditions"):
            ...
            initial_input_data = ds[config['variables']].isel(
                ...
                longitude=slice(*config['lon_range'], config['downsample']),
                latitude=slice(*config['lat_range'], config['downsample'])
            ).to_array().values
            # ==========================================================
            ...
            # Get the real time for the prediction period
    ...
    exit() # If the data file is not found, exit directly
...
if all_predictions_list:
    ...
    # Also use the correct dimension names when creating the Dataset
    output_ds = xr.Dataset(
        data_vars={
            'initial_condition': (('case', 'input_time_step', 'variable', 'latitude', 'longitude'), initial_conditions_arr),
            'prediction': (('case', 'lead_time_step', 'variable', 'latitude', 'longitude'), predictions_arr),
            'ground_truth': (('case', 'lead_time_step', 'variable', 'latitude', 'longitude'), ground_truth_arr),
        },
        coords={
            ...
            'latitude': ('latitude', lat_coords_real),
            'longitude': ('longitude', lon_coords_real),
        }
    )
    ...
    try:
        ...
        save_path = os.path.join(config['output_path'], f'inference_results_0125_{num_initial_conditions}_2020.nc')
