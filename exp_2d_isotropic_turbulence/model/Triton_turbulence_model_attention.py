...
class Triton_Turbulence_attention(nn.Module):
    def __init__(
        self,
        input_channel=1,
        ...
        super(Triton_Turbulence_attention, self).__init__()
        T = 1
        C = input_channel
...
if __name__ == '__main__':
    ...
    net = Triton_Turbulence_attention().to(device=device, dtype=torch.bfloat16)

    input = torch.randn(1, 1, 1, 128, 128).to(device=device, dtype=torch.bfloat16)
    output = net(input)
