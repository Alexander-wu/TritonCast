...
if local_rank == 0:
    for input_frames, output_frames in train_loader:
        # Dataloader Input shape: torch.Size([20, 1, 1, 128, 128]), Output shape: torch.Size([20, 1, 1, 128, 128])
...
if model_name in model_dict:
    ...
    raise ValueError(f"Model '{model_name}' is not defined in model_dict.")
...
def test(model, test_loader, criterion, device):
    ...
    with torch.no_grad():
        for inputs, targets in tqdm(test_loader, desc="Testing", disable=local_rank != 0):
            ...
            if local_rank == 0:
                # 保存时可以移除channel和time维度，恢复为 (B, H, W) 或 (B, T, H, W)
...
for epoch in range(num_epochs):
    ...
    if local_rank == 0:
        ...
        if val_loss < best_val_loss:
            ...
            logging.info(f'New best model saved with validation loss: {best_val_loss:.7f}')
...
if local_rank == 0:
    try:
        ...
        logging.error(f'Error during final testing: {e}')
