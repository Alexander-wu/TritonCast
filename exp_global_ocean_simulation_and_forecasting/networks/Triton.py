...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # 注意：SDPA 已内置缩放，这里保留 scale 但不再手动应用
    ...
    def forward(self, x):
        ...
        # SDPA 期望 (B, h, N, d)，不要再手动缩放（函数内部会做 1/sqrt(d)）
...
class Triton(nn.Module):
    def __init__(
        ...
        params,
        input_channel=97,
        spatial_hidden_dim=256, #>256
        output_channels=93,
        temporal_hidden_dim=512,# >512
    ...
    def custom_forward(self, input_state):
        input_state = input_state.unsqueeze(1)
        # input_state = input_state.unsqueeze(1).requires_grad_(True)
        # print('input_state.requires_grad is:', input_state.requires_grad)  # 检查输入张量是否正确
        ...
        # print('reshaped_input.requires_grad is:', reshaped_input.requires_grad)  # 检查输入张量是否正确
        ...
        # print('encoded_features.requires_grad is:', encoded_features.requires_grad)  # 检查中间张量是否正确
    ...
    def forward(self, input_state):
        if self.gradient_checkpointing and self.training:
            outputs = torch.utils.checkpoint.checkpoint(self.custom_forward, input_state, use_reentrant=False)
        else:
            outputs = self.custom_forward(input_state)
        return outputs
...
if __name__ == '__main__':
    ...
    net = Triton().to(device=device, dtype=torch.bfloat16)

    input = torch.randn(1, 101, 480, 960).to(device=device, dtype=torch.bfloat16)
    output = net(input)
