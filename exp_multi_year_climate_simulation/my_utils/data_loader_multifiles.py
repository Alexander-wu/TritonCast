...
import math
# import cv2
from my_utils.img_utils import reshape_fields, reshape_fields_ocean



def get_data_loader(params, files_pattern, files_pattern_ocean, distributed, train):
    dataset = GetDataset(params, files_pattern, files_pattern_ocean, train)
...
class GetDataset(Dataset):
    def __init__(self, params, location, location_ocean, train):
        self.params = params
        ...
        # self.climate_mean = np.load('/jizhicfs/Prometheus/gaoyuan/llm/ft_local/data/coupled_1.5_23layers/climate_mean_s_t_ssh.npy', mmap_mode='r')
    ...
    def __getitem__(self, global_idx):
        ...
        if self.params.multi_steps_finetune == 1:
            ...
            # climate_mean_ocean = self.climate_mean[(local_idx-self.dt*self.n_history)//4:(local_idx//4+1):self.dt, self.ocean_channels, :120, :240]
            ocean = reshape_fields_ocean( 
                    self.files_ocean[year_idx][(local_idx-self.dt*self.n_history):(local_idx+1):self.dt, self.ocean_channels, :120, :240], 
                    'ocean', 
                    self.params, 
                    self.train, 
                    self.normalize, 
            ...
            ocean = np.expand_dims(ocean, axis=0)
        ...
        # return inp, tar  
        # print(inp.shape, ocean.shape)
        return np.concatenate((inp, ocean), axis=0), tar 
