...
def setup(params):
    ...
    model = build_model("Triton", experiment="global_ocean_forecasting", params=params).to(device)
    model = load_model(model, params, checkpoint_file)
    model = model.to(device)

    print('model is ok')

    model_atmos = build_model("TritonAtmos", experiment="global_ocean_forecasting", params=params).to(device)
    ...
    climate_mean = np.load('/apdcephfs_qy3/share_301734960/easyluwu/gy/data/coupled_0.25_23layers/climate_mean_s_t_ssh.npy', mmap_mode='r')[:365, :, :, :]
    ...
    return valid_data_full, valid_data_full_atmos, model, model_atmos

    
def autoregressive_inference(params, init_condition, valid_data_full, valid_data_full_atmos, model, model_atmos): 
    device = torch.cuda.current_device() if torch.cuda.is_available() else 'cpu'
    ...
    with h5py.File(params.land_mask_path, 'r') as _f: 
        mask_data = torch.as_tensor(_f['fields'][:,out_channels, :720, :1440], dtype=bool)
    ...
    with torch.no_grad():
        for i in range(valid_data.shape[0]): 
            if i==0: # start of sequence, t0 --> t0'
                ...
                future_force = future_force * torch.from_numpy(params.stds_atmos[:,[65, 66, 67, 68]]).float().to(device) + torch.from_numpy(params.means_atmos[:,[65, 66, 67, 68]]).float().to(device)  
                future_force = (future_force - torch.from_numpy(params.means[:,[93, 94, 95, 96]]).float().to(device))/torch.from_numpy(params.stds[:,[93, 94, 95, 96]]).float().to(device)
                future_force = F.interpolate(future_force, size=(720, 1440), mode='bilinear', align_corners=False)
...
if __name__ == '__main__':
    ...
    params['best_checkpoint_path'] = os.path.join(expDir, 'training_checkpoints/best_ckpt_final.tar')
    ...
    valid_data_full, valid_data_full_atmos, model, model_atmos = setup(params)
    ...
    for i, ic_ in enumerate(ics):
        ...
        seq_real, seq_pred = autoregressive_inference(params, ic_, valid_data_full, valid_data_full_atmos, model, model_atmos)
        ...
        if i==0:
            ...
            f.create_dataset(
                    "ground_truth",
                    data=seq_real[:, :, [0, 12, 16, 20, 23, 35, 39, 43, 46, 58, 62, 66, 69, 81, 85, 89, 92], :, :],
                    maxshape=[None, prediction_length, n_out_channels, img_shape_x, img_shape_y], 
                    dtype=np.float32)
            f.create_dataset(
                    "predicted",       
                    data=seq_pred[:, :, [0, 12, 16, 20, 23, 35, 39, 43, 46, 58, 62, 66, 69, 81, 85, 89, 92], :, :], 
                    maxshape=[None, prediction_length, n_out_channels, img_shape_x, img_shape_y], 
                    dtype=np.float32)
            f.close()
        else:
            ...
            f["ground_truth"][-1:] = seq_real[:, :, [0, 12, 16, 20, 23, 35, 39, 43, 46, 58, 62, 66, 69, 81, 85, 89, 92], :, :] 

            f["predicted"].resize((f["predicted"].shape[0] + 1), axis = 0)
            f["predicted"][-1:] = seq_pred[:, :, [0, 12, 16, 20, 23, 35, 39, 43, 46, 58, 62, 66, 69, 81, 85, 89, 92], :, :]  
            f.close()
